...
class EndTurnConsole(messageService: (Input) => Unit, delayFromEndAttCtrl: () => Duration) extends VBox with Scaleable with PlayerListener with PhaseListener with UIController {
  ...
  def enableButton(): Unit = {
    this.setOnMouseClicked(() => messageService(EndTurn))
  }
  ...
  def onPhaseChange(oldPhase: Phase, newPhase: Phase): () => Unit = {
    ...
    def fromAny:Boolean = {
      val first = oldPhase match {case x if !x.isInstanceOf[Reinforcement] => true case _=> false}
    ...
    Unit match {
      case _ if fromAttack => () => {
        updateColor()
        openAnimation.setDelay(delayFromEndAttCtrl())
        openAnim(()=>{
          openAnimation.setDelay(new Duration(0))
          enableButton()
        })
      }
